...
3. Set the `TRANSPORT_TYPE` environment variable to "kafka" or "syncservice" to set which transport to use.
    ```
    $ export TRANSPORT_TYPE=...
    ```
4. If you chose Kafka for transport, set the following environment variables:
...
5. Otherwise, if you chose Sync-Service as transport, set the following:
...
6. Run the following command to deploy the `leaf-hub-spec-sync` to your leaf hub cluster:  
    ```
